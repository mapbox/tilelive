...
tilelive.load = function(uri, callback) {
    ...
    if (typeof uri === 'string') {
        ...
        // Obtain protocol from uri string
...
tilelive.copy = function(src, dst, opts, callback) {
    if (opts.scheme === 'list' && !opts.listStream) {
        return callback(new Error('You must provide a listStream'));
    }
    if (!dst && !opts.outStream) {
        return callback(new Error('You must provide either a dsturi or an output stream'));
    }
    if (!dst && opts.outStream) return out();
    
    // Load source and destination if URIs
    var q = queue();
  
    if (typeof src === 'string') q.defer(function(next) {
        tilelive.load(src, function(err, loaded) {
    ...
    if (typeof dst === 'string') q.defer(function(next) {
        tilelive.load(dst, function(err, loaded) {
            if (err) return next(err);
            dst = loaded;
            next();
        })
    ...
    q.await(function(err) {
        if (err) return callback(err);
        copy(src, dst, opts, function(err){
            if (err) return callback(err);
            return;
        });
    });
    ...
    function copy(src, dst, opts, callback) {
        ...
        if (opts.job) streamopts.job = opts.job;
        
        // Copy
        var get = tilelive.createReadStream(src, streamopts);
        var put = tilelive.createWriteStream(dst);
        get.on('error', function(err) { return(err); });
        put.on('error', function(err) { return(err); });
        ...
        // implement progress function if passed in
        if (opts.progress) prog.on('progress', function(p) { opts.progress(get.stats, p); });

        put.on('finish', function() { return callback; });
    };
    ...
    function out() {
        ...
        if (opts.scheme === 'list') {
            var file = opts.listStream;
            file.pipe(get).pipe(tilelive.serialize()).pipe(opts.outStream);
        } else {
            get.pipe(tilelive.serialize()).pipe(opts.outStream);
        }
    }
};

...
tilelive.copy = function(srcUri, dstUri, options, callback) {
    var prog = progress({
        objectMode: true,
        time: 100
    });
    var get = tilelive.createReadStream(srcUri, {
        type:options.scheme,
        minzoom:options.minzoom,
        maxzoom:options.maxzoom,
        bounds:options.bounds
    });
    var put = tilelive.createWriteStream(dstUri);
    get.on('error', function(err) { return(err); });
    put.on('error', function(err) { return(err); });
    get.on('length', prog.setLength);

    if (argv.scheme === 'list') {
        var file = fs.createReadStream(path.resolve(options.list));
        file.pipe(get).pipe(prog).pipe(put);
    } else {
        get.pipe(prog).pipe(put);
    }

    prog.on('progress', function(p) { report(get.stats, p); });

...
tilelive.validate = function(info) {
    ...
    for (var key in info) {
        ...
        case 'scheme':
            ...
            break;
        case 'minzoom':
            if (typeof val !== 'number' || val < 0 || val > 23 || Math.floor(val) !== val)
                return new Error('minzoom must be an integer between 0 and 23');
            break;
        case 'maxzoom':
            if (typeof val !== 'number' || val < 0 || val > 23 || Math.floor(val) !== val)
                return new Error('maxzoom must be an integer between 0 and 23');
            break;
        case 'name':
        case 'version':
        ...
        case 'center':
            ...
            if (val[2] < 0 || val[2] > 23 || Math.floor(val[2]) !== val[2])
                return new Error('center z value must be an integer between 0 and 23');
            break;
        case 'bounds':

...
tilelive.copy = function(src, dst, options, callback) {
    if (options.scheme === 'list' && !options.listStream) {
    ...
    if (!dst && options.outStream) return out(options);
    ...
    function out(options) {
        ...
        if (options.scheme === 'list') {
            var file = options.listStream;
            file.pipe(get).pipe(tilelive.serialize()).pipe(options.outStream);
        } else {
            get.pipe(tilelive.serialize()).pipe(options.outStream);
        }
    }
};

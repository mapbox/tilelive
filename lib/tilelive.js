...
tilelive.copy = function(source, sink, bbox, minZ, maxZ, concurrency, callback) {
    ...
    if (typeof concurrency === 'function') { callback = concurrency; concurrency = undefined; }
    if (typeof concurrency !== 'number') concurrency = 100;
    ...
    var q = new Queue(function(coords, next) {
        ...
        source.getTile(coords[0], coords[1], coords[2], function(err, data) {
            if (err) {
                ...
                sink.putTile(coords[0], coords[1], coords[2], data, function(err) {
                    if (err) {
                        action.failed++;
                        action.emit('warning', err);
                    } else {

...
tilelive.copy = function(source, sink, concurrency, bbox, minZ, maxZ, callback) {
    if (typeof source.getTile !== 'function') throw new Error('source must be a tilesource');
    if (typeof source.putTile !== 'function') throw new Error('sink must be a tilesink');
    if (!Array.isArray(bbox) || bbox.length !== 4) throw new Error('bbox must have four lat/long coordinates');
    if (typeof concurrency !== 'number' || concurrency < 1) throw new Error('concurrency must be >= 1');
    if (typeof minZ !== 'number') throw new Error('minZ must be a number');
    if (typeof maxZ !== 'number') throw new Error('maxZ must be a number');
    if (typeof callback !== 'function') throw new Error('callback must be a function');

    if (minZ < 0) throw new Error('minZ must be >= 0');
    if (maxZ > 22) throw new Error('maxZ must be <= 22');
    if (minZ > maxZ) throw new Error('maxZ must be >= minZ');

    // TODO: Convert bbox/minZ to minX/maxX/min
    var minX = 0, maxX = 3, minY = 5, maxY = 9;

    // Sets z, x and y to the next tile.
    var x = minX - 1, y = minY, z = minZ;
    function id() {
        x++;
        if (x > maxX) {
            x = minX; y++;
    ...
    var q = new Queue(function(coords, next) {
        ...
        // Actually copy tiles.
        source.getTile(coords[0], coords[1], coords[2], function(err, data) {
            if (err) {
                console.trace(err.stack);
                return next();
            } else {
                sink.putTile(coords[0], coords[1], coords[2], function(err) {
    ...
    sink.startWriting(function(err) {
        ...
        q.on('empty', function() {
            if (!callback) throw Error('Callback was already called');
            var cb = callback;
            callback = null; // Prevent accidental double calls
            sink.stopWriting(cb);
        });

        // Fill the queue.
        for (var i = concurrency; i && id(); i--) {
            q.add([z, x, y]);
        }
    });
};

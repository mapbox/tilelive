...
tilelive.copy = function(srcUri, dstUri, opts, callback) {
    var prog = progress({
        objectMode: true,
        time: 100
    });
    ...
    if (opts.job) stream_options.job = opts.job;
    
    var get = tilelive.createReadStream(opts.src, stream_options);
    var put = tilelive.createWriteStream(opts.dst);
    get.on('error', function(err) { return(err); });
    put.on('error', function(err) { return(err); });
    get.on('length', prog.setLength);

    if (opts.scheme === 'list') {
        var file = fs.createReadStream(path.resolve(opts.list));

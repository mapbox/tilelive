...
Pyramid.prototype._params = function(callback) {
    ...
    stream.source.getInfo(function(err, info) {
        ...
        //     var dy = (stream.bounds[3] - stream.bounds[1]) / stream.job.total;
        //     stream.bounds[0] = stream.bounds[0] + (stream.job.num - 1) * dx;
        //     stream.bounds[2] = stream.bounds[0] + stream.job.num * dx;
        // }
        var bbox, dx;
        stream.bboxes = {};
        for (var z = stream.minzoom; z <= stream.maxzoom; z++) {
            bbox = sm.xyz(stream.bounds, z);
            numXTiles = bbox.maxX - bbox.minX + 1;
            // var numTiles =
            //     (bbox.maxX - bbox.minX + 1) *
            //     (bbox.maxY - bbox.minY + 1);

            if (stream.job && numXTiles < stream.job.num) continue;

            else if (stream.job) {
                numJobs = Math.min(numXTiles, stream.job.total);
                dx = Math.ceil(numXTiles / numJobs);
                stream.bboxes[z] = {
                    minX: Math.min(bbox.minX + (stream.job.num - 1) * dx, bbox.maxX),
                    minY: bbox.minY,
                    maxX: Math.max(Math.min(bbox.minX + stream.job.num * dx - 1, bbox.maxX), 0),
                    maxY: bbox.maxY
                };
            }

            else stream.bboxes[z] = bbox;
        ...
        stream.minzoom = Object.keys(stream.bboxes).reduce(function(memo, z) {
            memo = Number(z) < memo ? z : memo;
            return memo;
        });

        if (stream.job) console.log(stream.job.num, stream.bboxes);

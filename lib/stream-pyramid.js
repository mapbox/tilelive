    ...
    multiread(stream, function get(push) {
        ...
        function done(err, buffer) {
            ...
            if (err && !(/does not exist$/).test(err.message)) {
                ...
                tile = new Tile(z, x, y);
                // stream.empty flag marks whether the stream has only
                // consisted of empty tiles to this point. Until the
                // first tile with content has been seen pyramid scheme
                // will not skip tiles.
                var sum = 0;
                if (stream.empty) {

...
FileScheme.prototype.error = function(tile, err) {
    ...
    this.stats.failed++;
    this.task.error(tile, err, this.next);
};

FileScheme.prototype.unique = function(tile, data) {
    var index = this.pending.indexOf(tile);
    if (index >= 0) {
        this.pending.splice(index, 1);
        this.stats.pending--;
    ...
    this.stats.unique++;
    this.task.unique(tile, data, this.next);
};
...
FileScheme.prototype.skip = function(tile) {
    ...
    this.stats.skipped++;
    this.task.skip(tile, this.next);
};

FileScheme.prototype.duplicate = function(tile, data, key) {
    var index = this.pending.indexOf(tile);
    if (index >= 0) {
        this.pending.splice(index, 1);
        this.stats.pending--;
    ...
    this.task.duplicate(tile, data, key, this.next);
};

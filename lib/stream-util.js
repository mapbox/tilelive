...
Tile.prototype.serialize = function() {
    var obj = {
        z: isNaN(this.z) ? null : Number(this.z),
        x: isNaN(this.x) ? null : Number(this.x),
        y: isNaN(this.y) ? null : Number(this.y),
        buffer: this.buffer instanceof Buffer ? this.buffer.toString('base64') : null
    };
...
Tile.prototype.deserialize = function(data) {
    ...
    Tile.call(this, obj.z, obj.x, obj.y, buf);
    return this;
};
...
function serializedType(data) {
    if (data.indexOf('{"z":') === 0) return new Tile();
    else if (data.indexOf('{') === 0) return new Info();
    throw new DeserializationError('Invalid data');
}

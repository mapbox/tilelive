...
function getTileRetry(source, z, x, y, callback, err) {
    callback._retry = callback._retry || 0;
    if (callback._retry > 9) return callback(err);

    setTimeout(function() {
        source.getTile(z, x, y, function(err, buffer) {
            if (err && !(/does not exist$/).test(err.message)) {
                callback._retry++;
                return getTileRetry(source, z, x, y, callback, err);
            }
            callback(err, buffer);
        });
    }, 500 * Math.pow(callback._retry, 2));
}

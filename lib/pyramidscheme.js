...
PyramidScheme.unserialize = function(state) {
    ...
    var pending = Scheme.unserializeTiles(state.pending);
    scheme.stack = Scheme.unserializeTiles(state.stack).concat(pending);
...
PyramidScheme.prototype.nextMetatile = function() {
    ...
    if (pos.y > bounds.maxY) return false; // Top level exhausted.
...
PyramidScheme.prototype.next = function() {
    ...
    // all of its members.
    while (this.pending.length < this.concurrency || this.box.length) {
        var tile;
        if (this.box.length) {

...
Server.prototype.serve = function(options, callback) {
    ...
    // We don't use .bind() here because of an issue with node-pool's
    // callback arg length autodetection:
    // https://github.com/coopernurse/node-pool/issues/16
    var that = this;
    this.acquire(options.datasource, function(err, resource) {
        if (err) return callback(err, null);
        that.backend.serve(resource, options, function(err, data) {
            that.release(options.datasource, resource);
            callback(err, data);
        });
    });
};

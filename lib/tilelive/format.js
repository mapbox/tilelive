var Format = {
    ...
    grid: {
         ...
         */
        'render': function(tile, callback) {
            console.log(tile.format);
            var format = tile.format.match(/([^\.]+)\.grid\.json/);
            if (format[1]) {
                var field = (new Buffer(format[1], 'base64')).toString('utf-8');
                tile.map.mapnik_map_acquire(function(err, map) {
                    if (err) return callback(err);
                    map.zoom_to_box(tile.bbox);
                    try {
                        callback(null, [map.generate_hit_grid(0, 4, field),

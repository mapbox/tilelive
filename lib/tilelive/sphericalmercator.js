...
SphericalMercator.prototype.bbox_to_xyz = function(bbox, zoom, tms_style) {
    ...
    var bounds = {
        minX: Math.round(px_ll[0]/this.size),
        minY: Math.round(px_ur[1]/this.size),
        maxX: Math.round(px_ur[0]/this.size),
        maxY: Math.round(px_ll[1]/this.size)
    };
    if (tms_style) {
        var tms = {
            minY: (Math.pow(2, zoom)) - bounds.maxY,
            maxY: (Math.pow(2, zoom)) - bounds.minY
        };

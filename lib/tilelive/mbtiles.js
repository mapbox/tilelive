    ...
    Step(
        ...
        function() {
            ...
            that.db.prepare(
                ...
                + 'tile_id TEXT, '
                + 'grid_id TEXT);',
                function(err, statement) { console.log(err); statement.step(next); }
            );
        },
        function() {
            var next = this;
...
MBTiles.prototype.insertGrids = function(tiles, renders, compress, callback) {
    ...
    for (var i = 0; i < tiles.length; i++) {
        ...
        if (compress) {
            ...
            grid_id = crypto.createHash('md5').update(renders[i]).digest('hex');
        } else {
    ...
    that.db.prepare(
        ...
        function(err, statement) {
            if (statement) {
                ...
                console.log('inserted grid data');
                callback(err);
            }
        }
    );

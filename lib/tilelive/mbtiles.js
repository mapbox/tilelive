...
// Wrapper around `statement.step()` for single-row result sets.
...
// @TODO merge `step()` and `rows()` into a single function and write
// equivalent handling multi-row handling for `bind()`.
...
MBTiles.prototype.formatter = function(callback) {
    ...
    Step(
        ...
        function(err, row) {
            if (err) callback(err);
            else if (!row || !row.value) callback('empty row');

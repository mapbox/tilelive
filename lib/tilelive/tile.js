...
Tile.prototype.render = function(callback) {
    ...
    Step(
        function() {
            // for the time being, jury-rig type
            Pool.acquire('map', that.datasource, that.options, this);
        },
        function(err, res) {
        ...
        function(err, data) {
            Pool.release('map', that.datasource, resource);
            callback(err, data);
        }
    );
};

...
// - `bbox` {Array} bounding box coordinates in the proper units
//    (default is 900913) and in the order `[w, s, e, n]`
...
// - `srs` {String} projection for provided bbox (WGS84|900913).
//    Defaults to 900913.
var TileBatch = function(options) {
    ...
    this.options.srs = options.srs || '900913';
    ...
    for (var z = this.minzoom; z <= this.maxzoom; z++) {
        this.zoomBounds[z] = this.bbox_to_xyz(this.options.bbox, z, true, this.options.srs);
    ...
    if (!this.options.metadata.bounds) {
        if (this.options.srs === '900913') {
            this.options.metadata.bounds = this.bbox_convert(this.options.bbox, 'WGS84')
                .join(',');
        } else {
            this.options.metadata.bounds = this.options.bbox.join(',');
        }
    }

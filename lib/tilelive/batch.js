...
// Render a job's full of interaction data.
// -----------------------------
// Calls its callback with `err`, `tiles`.
TileBatch.prototype.renderInteractionChunk = function(callback) {
    // @TODO handle tile compression
    var tiles = this.next(this.options.batchsize);
    var that = this;
    ...
    Step(
        ...
        function(err, renders) {
            if (err) { return this(err) }
            var renders = _.pluck(renders, 0);
            that.mbtiles.insertGrid(tiles, renders, that.options.compress, this);
        },
        function(err) {
            callback(null, tiles);
        }

...
var Batch = function(options) {
    ...
    // ensure that the metatile value is a power of 2
    var is_2p = options.metatile && !(options.metatile & (options.metatile - 1));
    if (!is_2p) {
        throw new Error('`metatile` value is not valid "' + options.metatile + '" (it must be a power of 2)');
    }
};
                    ...
                    var group = this.group();
                      for (var j = 0; j < renders.length; j++) {
                          ...
                          batch.storage.store(
                              'tiles',
                              resource,
                              _(render).map(function(r, i) {
                                  return _({}).extend(r[2], {data: r[0]});
                              }),
                              group()
                          );
                      }
...
Batch.prototype.next = function(count) {
    ...
    for (var z = this.curZ; z <= this.maxzoom; z++) {
        ...
        var metasize = 1;
        if (this.options.metatile > 1)
            metasize = Math.min(this.options.metatile, 1 << z);

        for (var x = this.curX; x <= this.zoomBounds[z].maxX; x=x+metasize) {
            if ((x < 0) || (x >= Math.pow(2,z))) {
               continue;
            }
            for (var y = this.curY; y <= this.zoomBounds[z].maxY; y=y+metasize) {
                ...
                this.curY = y;
                this.curZ = z;
                if (tile_info.length < count) {
                    var info = {z: z, x: x, y: y,
                                metatile: this.options.metatile,
                                metasize: metasize };
                    tile_info.push(info);
                    this.tilesCurrent=this.tilesCurrent+(metasize*metasize);
                } else if (tile_info.length === count) {
                    return tile_info;
                }
            }
    ...
    return tile_info.length ? tile_info : false;
};
